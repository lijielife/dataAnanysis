<template>
    ...
    <system-layout
        ...
        <system-userInfo slot="userInfo" :userInfo="userInfo"></system-userInfo>
        <nb-layout class="page-container" ref="container" slot="pageContent"></nb-layout>
        <!--<system-tabs slot="tabs" :tabList="tabList" @delTab="delTab"
        @delAllTab="delAllTab" ></system-tabs>-->
        <system-menu
            ref="systemMenu"
            slot="menu"
            :menus="menus"
...
<script>
            ...
            menuStatusChange(menuCollapsed) {
                if (menuCollapsed === false) {
                    ...
                    this
                        ...
                        .$refs
                        .menu
                        .toggleAllSubmenu(false);
                }
                this
                    .$store
                    .commit('setMenuStatus', menuCollapsed);

                // 处理悬浮菜单闪现的问题
                this
                    .$store
                    .commit('setHideInlieSubmenuStatus', true);
                setTimeout(() => {
                    this
                        .$store
                        .commit('setHideInlieSubmenuStatus', false);
                }, 500);
            },
            async delTab(key) {
                this
    ...
    export default class Wrap extends Engine.WebView {
        ...
        syetemMenuChange(val) {
            this
                .$store
                .commit('setMenuSelectKey', val);
            this
                .$store
                .commit('setBreadcrumb', val);
            this
                .$store
                .commit('setTabList', val);
        }
    }
</script>