    ...
    <div id="user-info__area">
        ...
        >
            <div class="user__info">
                <p class="user__item user__avatar" 
                :style="style"
                ></p>
                <p class="user__item user__name">{{userName}}</p>
            </div>
            <nb-menu slot="dropdown-menu" @input="_dropdownClick">
                <nb-menu-item name="info"><icon type="user"></icon>用户信息</nb-menu-item>
                <nb-menu-item name="logout"><icon type="logout"></icon>退出登录</nb-menu-item>
            </nb-menu>
        </nb-dropdown>
    </div>
    <div class="user-info__tool">
        <div @click="handleFullScreen" v-if="showFullScreenBtn" class="full-screen-btn-con">
            <nb-tooltip :content="isFullScreen ? '退出全屏' : '全屏'" placement="bottom">
                <i :class="{ 'ion-arrow-expand': !isFullScreen, 'ion-arrow-shrink': isFullScreen}"></i>
            </nb-tooltip>
        </div>
    </div>
  </div>
...
export default class Avatar {
    get style() {
        return {
            backgroundImage: this.avatarSrc ? `url(${this.avatarSrc})` : undefined,
